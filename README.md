...
Movement Analysis uses MediaPipe and OpenCV on the local Python server. It identifies players, asks the user to confirm the target player when needed, analyzes full-match body movement, and saves annotated frames. Video files are not sent to an external analysis service.
...
Redeploy after saving the variables. Accounts, matches, goals, calendar entries, and training plans will sync directly to JSONBin. MediaPipe/OpenCV video analysis requires a separately deployed Python service.
