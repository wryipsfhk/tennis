...
function scoreFormatLabel(match){if(match.scoreFormat!=='custom-points')return '';const lead=match.customScoring?.winBy??1;return `Custom points · first to ${match.customScoring?.target||'—'}${lead===0?' wins':`, win by ${lead}`}`;}
...
function syncFirstToTarget(){const toggle=document.querySelector('#customFirstToTarget'),winBy=document.querySelector('#customWinBy');if(toggle.checked)winBy.value='0';winBy.readOnly=toggle.checked;winBy.classList.toggle('is-first-to-target',toggle.checked);}
...
function editCurrentMatch() {
  ...
  if(format==='custom-points'){const score=match.sets?.[0]||{};document.querySelector('#customTarget').value=match.customScoring?.target||10;document.querySelector('#customWinBy').value=match.customScoring?.winBy??2;document.querySelector('#customFirstToTarget').checked=Number(match.customScoring?.winBy)===0;syncFirstToTarget();document.querySelector('#customPlayerScore').value=score.player??'';document.querySelector('#customOpponentScore').value=score.opponent??'';}
...
document.querySelector('#customFirstToTarget').addEventListener('change',event=>{if(!event.currentTarget.checked&&document.querySelector('#customWinBy').value==='0')document.querySelector('#customWinBy').value='2';syncFirstToTarget();});document.querySelector('#customWinBy').addEventListener('input',event=>{const toggle=document.querySelector('#customFirstToTarget');toggle.checked=event.currentTarget.value==='0';syncFirstToTarget();});
