...
document.querySelector('#togglePassword').addEventListener('click',event=>{const input=document.querySelector('#password');input.type=input.type==='password'?'text':'password';event.currentTarget.textContent=input.type==='password'?'Show':'Hide';});document.querySelector('#forgotOpen').addEventListener('click',()=>forgotModal.showModal());document.querySelector('#forgotForm .modal-close').addEventListener('click',()=>forgotModal.close());
document.querySelector('#forgotForm').addEventListener('submit',async event=>{event.preventDefault();const email=document.querySelector('#resetEmail').value.trim().toLowerCase(),data=accounts();if(!data[email]){setError('#resetError','No account was found for this email.');return;}data[email].passwordHash=await hashPassword(document.querySelector('#resetPassword').value);saveAccounts(data);forgotModal.close();event.currentTarget.reset();document.querySelector('#email').value=email;showToast('Password updated');});
