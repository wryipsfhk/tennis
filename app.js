...
async function refreshAnalysisCapability(force=false) {
  ...
  element.className='analysis-capability is-checking';
  element.querySelector('span').textContent='Checking whether the Python analysis service is available…';
  try{
    const response=await fetch('/api/analysis-capabilities',{cache:'no-store'});
    if(!response.ok)throw new Error('offline');
    const result=await response.json();
    element.className='analysis-capability is-online';
    element.querySelector('span').textContent=`Analyzer online · ${result.multiWindow?'multiple movement windows':'pose review'} · no ball/contact detection`;
  }catch{
    element.className='analysis-capability is-offline';
    element.querySelector('span').textContent='Video analysis backend is offline. Match records still work; analysis needs the separate Python service.';
  }
}
...
function englishMistake(segment){const label=(segment.check?.label||segment.label||'').toLowerCase();if(label.includes('rotation'))return{title:'Limited visible upper-body rotation',evidence:'Across the selected body-motion window, the projected shoulder line changed less than the review threshold.',advice:'Complete the shoulder and hip turn before letting the arm follow.'};if(label.includes('balance'))return{title:'Balance moved outside the base of support',evidence:'In this exact window, the projected hip centre moved beyond the area supported by both visible feet.',advice:'Lower your centre of mass and hold the finish for two seconds before beginning the recovery step.'};if(label.includes('stance'))return{title:'Stance width limited stability or recovery',evidence:'The visible ankle-to-ankle width was outside the comparison range relative to the stable shoulder-width reference for this window.',advice:'Adjust toward a comfortable shoulder-width base, stay active with small steps, and recover immediately after the swing.'};if(label.includes('lower-body')||label.includes('knee'))return{title:'Limited visible knee flexion',evidence:'The knees remained relatively straight during this exact body-motion window.',advice:'Use a light knee bend during preparation, followed by a natural extension through the swing.'};if(label.includes('swing'))return{title:'Short visible swing path',evidence:'The racket-side wrist travelled a short distance relative to shoulder width during this movement window.',advice:'Rehearse a slow, continuous shadow swing before gradually adding speed.'};if(label.includes('posture')||label.includes('head'))return{title:'Large vertical posture change',evidence:'The visible nose keypoint changed height sharply relative to shoulder-to-hip length within this exact window.',advice:'Rehearse beside a fixed visual marker and keep your level steady until the forward movement is established.'};return{title:'Visible movement pattern',evidence:'This observation comes from the selected player’s body-keypoint path across the frames shown here.',advice:'Treat it as an improvement to test. Compare it with the replay and how the movement felt.'};}
...
async function analyzeSelectedPlayer(candidate){const loading=document.querySelector('#analysisLoading'),button=document.querySelector('#analyzeButton');loading.hidden=false;document.querySelector('#analysisLoadingTitle').textContent='Tracking the selected player across distinct movement windows';button.disabled=true;setError('#analysisError');try{const response=await fetch('/api/analyze-match',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({uploadId:pendingVideoAnalysis.uploadId,bbox:candidate.bbox})});const result=await response.json().catch(()=>({}));if(!response.ok)throw new Error(response.status===404?'The Python analysis backend is not deployed at this address. Match records still work, but video analysis needs a separate server.':result.error||'The full-match analysis could not be completed.');result.createdAt=new Date().toISOString();result.selectedPlayer=candidate.label;result.uploadId=result.uploadId||pendingVideoAnalysis.uploadId;result.playerBBox=result.playerBBox||candidate.bbox;updateAccount(account=>{account.analyses=account.analyses||[];account.analyses.unshift(result);});renderAnalysis(result);renderAnalysisHistory();document.querySelector('#playerSelection').hidden=true;button.hidden=false;showToast('Multi-window movement review complete');}catch(error){setError('#analysisError',error.message||'The full-match analysis could not be completed. Please try again.');}finally{loading.hidden=true;button.disabled=false;}}
...
document.querySelector('#motionForm').addEventListener('submit',async event=>{event.preventDefault();const file=document.querySelector('#motionVideo').files[0];if(!file)return;if(file.size>250*1024*1024){setError('#analysisError','Video must be no larger than 250 MB.');return;}const loading=document.querySelector('#analysisLoading'),button=document.querySelector('#analyzeButton');loading.hidden=false;document.querySelector('#analysisLoadingTitle').textContent='Identifying player tracks across the video';button.disabled=true;setError('#analysisError');const form=new FormData();form.append('video',file);try{const response=await fetch('/api/prepare-analysis',{method:'POST',body:form});const result=await response.json().catch(()=>({}));if(!response.ok)throw new Error(response.status===404?'The Python analysis backend is not deployed at this address. Match records still work, but video analysis needs a separate server.':result.error||'Player detection could not be completed.');pendingVideoAnalysis=result;if(result.candidates.length===1){await analyzeSelectedPlayer(result.candidates[0]);return;}document.querySelector('#playerCandidates').innerHTML=result.candidates.map((candidate,index)=>`<label class="player-candidate"><input type="radio" name="selectedPlayer" value="${index}" ${index===0?'checked':''}/><span><img src="${escapeHtml(candidate.thumbnail)}" alt="${escapeHtml(candidate.label)}"/><strong>${escapeHtml(candidate.label)}</strong><small>${candidate.detectionFrames?`Seen in ${candidate.detectionFrames} sampled frames`:`Player ${index+1}`}</small></span></label>`).join('');document.querySelector('#playerSelection').hidden=false;button.hidden=true;showToast('Select the player to analyze');}catch(error){setError('#analysisError',error.message||'Player detection could not be completed. Please try again.');}finally{loading.hidden=true;button.disabled=false;}});
